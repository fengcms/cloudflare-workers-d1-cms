...
async function main() {
  ...
  const userResponse = await request('POST', '/user', {
    ...
    body: {
      username: `test_user_prod_${Date.now()}`,
  ...
  const editorResponse = await request('POST', '/user', {
    ...
    body: {
      username: `test_editor_prod_${Date.now()}`,
      password: sha256('test123'),
      nickname: '生产测试编辑',
      type: 'EDITOR'
    }
  ...
  const userArticleResponse = await request('POST', '/article', {
    ...
    body: {
      title: 'USER 用户创建的生产文章',
      channel_id: channelId,
      content: '这是 USER 用户在生产环境创建的文章内容',
      type: 'NORMAL'
    }
  });
  ...
  const editorArticleResponse = await request('POST', '/article', {
    ...
    body: {
      title: 'EDITOR 用户创建的生产文章',
      channel_id: channelId,
      content: '这是 EDITOR 用户在生产环境创建的文章内容',
      type: 'NORMAL'
    }
  });
  ...
  const adminArticleResponse = await request('POST', '/article', {
    ...
    body: {
      title: 'SUPERMANAGE 用户创建的生产文章',
      channel_id: channelId,
      content: '这是 SUPERMANAGE 用户在生产环境创建的文章内容',
      type: 'NORMAL'
    }
  });
  ...
  if (adminArticle.status === 'NORMAL') {
    ...
    console.log(`❌ 验证失败：SUPERMANAGE 用户创建的文章状态应为 NORMAL，实际为 ${adminArticle.status}`);
  }
