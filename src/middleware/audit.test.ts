...
describe('审计日志中间件', () => {
  ...
  describe('用户上下文提取', () => {
    it('应该提取认证用户信息', async () => {
      app.post('/api/v1/articles', async (c, next) => {
  ...
  describe('站点上下文提取', () => {
    ...
    it('应该处理缺少站点上下文的情况', async () => {
      app.post('/api/v1/system', async (c, next) => {
  ...
  describe('资源信息提取', () => {
    ...
    it('应该识别频道模块', async () => {
      app.post('/api/v1/channels', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
    ...
    it('应该提取资源ID', async () => {
      app.put('/api/v1/articles/456', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
    ...
    it('应该处理未知模块（使用 SYSTEM）', async () => {
      app.post('/api/v1/unknown', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
  ...
  describe('错误处理', () => {
    it('不应该阻塞请求（即使日志记录失败）', async () => {
      ...
      failApp.post('/api/v1/articles', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
    ...
    it('应该处理缺少数据库的情况', async () => {
      ...
      noDbApp.post('/api/v1/articles', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
  ...
  describe('日志内容格式', () => {
    it('应该生成正确的日志内容（带资源ID）', async () => {
      app.put('/api/v1/articles/789', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
    ...
    it('应该生成正确的日志内容（不带资源ID）', async () => {
      app.post('/api/v1/users', async (c, next) => {
        c.set('authContext', { userId: 1, username: 'testuser', type: 'USER', siteId: 1 })
        c.set('siteContext', { siteId: 1 })
        await next()
      }, (c) => c.json({ success: true }))
