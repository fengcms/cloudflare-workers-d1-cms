...
import { EditorEnum, GenderEnum, StatusEnum, UserTypeEnum } from '../types'
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (userData) => {
            ...
            // 从数据库查询
            const dbUser1 = db.select().from(users).where(eq(users.id, user1.id)).get()

            const dbUser2 = db.select().from(users).where(eq(users.id, user2.id)).get()
