...
describe('属性 8: 角色权限层级', () => {
  ...
  it('SUPERMANAGE 应该对所有操作拥有最高权限', () => {
    fc.assert(
      fc.property(roleArbitrary, (requiredRole) => {
  ...
  it('MANAGE 应该拥有比 EDITOR 和 USER 更高或相等的权限', () => {
    fc.assert(
      fc.property(fc.constantFrom(UserTypeEnum.EDITOR, UserTypeEnum.USER), (lowerRole) => {
  ...
  it('EDITOR 应该拥有比 USER 更高或相等的权限', () => {
    fc.assert(
      fc.property(fc.constant(UserTypeEnum.USER), (userRole) => {
  ...
  it('权限检查应该与角色级别一致', () => {
    fc.assert(
      fc.property(roleArbitrary, roleArbitrary, (userRole, requiredRole) => {
